...
Cons=[Cons,Cons_Op];
% size(Cons_Op) %80, 40 eq 1600 ineq
% size(Cons)
...
Cons=[Cons,Cons_ST];
% size(Cons_ST) %4800, 4800 eq (4880 eqs)
% size(Cons)
...
Cons=[Cons,Cons_Load];
% size(Cons_Load) % 1000 eq, 1680 ineq (5880)
% size(Cons)
...
%% Cons12: Fixed the topology and test voltage
load('Case25_noCuts_noHeu_noV_nox0_withDE3_realf12_noSCF_Gap5.mat','s_x1','s_y1','s_be1');
Cons_Fixed=[x==s_x1];
for C_l=1:L
    Cons_Fixed=[Cons_Fixed,y(:,C_l)==s_y1(:,C_l)];
    Cons_Fixed=[Cons_Fixed,be(:,C_l,1)==s_be1(:,C_l,1),be(:,C_l,2)==s_be1(:,C_l,2)];
end
Cons=[Cons,Cons_Fixed];

%% Set initial guess of x,y and be_Nodes to values in "XXXX.mat"
...
%     Cons_Vol=[Cons_Vol,v_min<=v(:,C_l)<=v_max];
%     Cons_Vol=[Cons_Vol,v(N_Subs,C_l)==v_max];
%     Cons_Vol=[Cons_Vol,-(1-s_y1(:,C_l))*M<=s_f1(:,C_l).*z-Full_I'*v(:,C_l)<=(1-s_y1(:,C_l))*M];
% end
% s_v1=value(v);
