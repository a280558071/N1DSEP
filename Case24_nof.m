...
%% solve the problem
ops=sdpsettings('solver','gurobi','verbose',2,'cplex.mip.display',3,'usex0',0);
...
for i=1:5 % Contigency i happens
    ...
    highlight(pi(i),s_temp(find(s_x==0)),t_temp(find(s_x==0)),'LineStyle','--','LineWidth',1);
    highlight(pi(i),s_temp(find(s_x==1)),t_temp(find(s_x==1)),'LineWidth',4); % bold line denotes the newly-built line
    highlight(pi(i),s_temp(i),t_temp(i),'EdgeColor','k','LineStyle','-','LineWidth',4);  % black line denotes the outage line
    highlight(pi(i),s_temp(find(s_y(:,i)==1)),t_temp(find(s_y(:,i)==1)),'EdgeColor','r','LineWidth',6,'LineStyle','-');
end
